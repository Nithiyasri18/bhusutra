...
import { landRecords, recordDistricts } from '../data/records'
...
export default function Records() {
  ...
  const filteredRecords = useMemo(() => landRecords.filter((record) => {
  ...
  }), [district, risk, search, status, verification, view])
  ...
  return <Layout title="Land records">
    <div className="dashboard-intro"><div><p className="text-sm text-slate-500">Review the seeded land-record register and focus on records that need officer attention.</p><p className="text-xs text-slate-400 mt-1">{filteredRecords.length} of {landRecords.length} records shown</p></div><Link to="/upload" className="primary-button">+ Upload new record</Link></div>
    <section className="panel">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-5 gap-3 mb-5">
        ...
        <label className="field-label">District<select value={district} onChange={(event) => setDistrict(event.target.value)} className="field-input"><option>All districts</option>{recordDistricts.map((item) => <option key={item}>{item}</option>)}</select></label>
