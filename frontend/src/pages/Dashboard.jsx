...
export default function Dashboard() {
  ...
  return <Layout title="Command dashboard">
    ...
    <div className="kpi-grid">{data.kpis.map((kpi) => <div className={`kpi-card kpi-${kpi.tone}`} key={kpi.label}><div className="kpi-top"><span>{kpi.label}</span><span className="kpi-arrow">&#8599;</span></div><div className="kpi-value">{kpi.value}</div><div className="kpi-bottom"><span className="kpi-change">{kpi.change}</span><span>{kpi.note}</span></div></div>)}</div>
