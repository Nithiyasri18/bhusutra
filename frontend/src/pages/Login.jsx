...
const demoAccounts = [
  { role: 'Administrator', email: 'admin@bhusutra.gov.in' },
  { role: 'Verification Officer', email: 'verifier@bhusutra.gov.in' },
  { role: 'District Officer', email: 'officer@bhusutra.gov.in' },
]
...
export default function Login() {
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      localStorage.setItem('bhusutra_role', email.startsWith('admin') ? 'Administrator' : email.startsWith('officer') ? 'District Officer' : 'Verification Officer')
    ...
    <main className="login-page">
      <section className="login-visual">
        <div className="login-visual-content">
          ...
          <div className="mt-auto max-w-lg"><div className="eyebrow light">State Land Records Department</div><h1 className="display-title">One source of truth for every parcel.</h1><p className="mt-5 text-white/65 text-base leading-7">AI-assisted validation for accurate, transparent and trusted land records across the state.</p><div className="mt-10 flex gap-8"><div><div className="text-2xl font-bold">18,642</div><div className="text-xs text-white/50 mt-1">Records processed</div></div><div><div className="text-2xl font-bold">92.4</div><div className="text-xs text-white/50 mt-1">Average trust score</div></div><div><div className="text-2xl font-bold">18</div><div className="text-xs text-white/50 mt-1">Districts onboarded</div></div></div></div>
        </div>
      ...
      <section className="login-form-side">
        <div className="login-form-wrap">
          ...
          <div className="demo-box"><div className="flex items-center justify-between"><span className="text-[11px] uppercase tracking-[0.14em] font-bold text-[#55708b]">Demo access</span><span className="demo-status">* LIVE DEMO</span></div><p className="text-xs text-slate-500 mt-2">Select a role to continue with seeded prototype data.</p><div className="mt-3 space-y-1">{demoAccounts.map((account) => <button type="button" key={account.email} onClick={() => { setEmail(account.email); setPassword('demo123') }} className="demo-account"><span>{account.role}</span><span>{account.email}</span></button>)}</div></div>
